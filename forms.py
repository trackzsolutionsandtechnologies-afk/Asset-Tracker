        ...
        if assets_df.empty:
            ...
            if filtered_df.empty:
                ...
                if "Purchase Cost" in available_columns:
                    filtered_df["Purchase Cost"] = pd.to_numeric(
                        filtered_df["Purchase Cost"].replace("", 0).astype(str).str.replace(",", ""),
                        errors="coerce",
                    ).fillna(0.0)
                if "Purchase Date" in available_columns:
                    filtered_df["Purchase Date"] = pd.to_datetime(
                        filtered_df["Purchase Date"], errors="coerce"
                    ).dt.date
                ...
                if save_clicked:
                    if edited_df.equals(display_df):
                        ...
                        editable_columns = [col for col in editor_columns if col != "Asset ID" and col in assets_df.columns]
                        ...
                        for asset_id, edited_row in edited_indexed.iterrows():
                            ...
                            match_rows = assets_df[
                                assets_df["Asset ID"].astype(str).str.strip() == str(asset_id).strip()
                            ]
                            if match_rows.empty:
                                continue
